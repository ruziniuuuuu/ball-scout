...
import { ApiResponse, NewsArticle } from '../../shared/types.ts';
...
Deno.test('News API Tests', async (t) => {
  ...
  await t.step(
    'GET /api/v1/news - should handle pagination parameters',
    async () => {
      const request = await superoak(app);
      const response = await request
  ...
  await t.step(
    'GET /api/v1/news - should handle invalid parameters',
    async () => {
      const request = await superoak(app);
      const response = await request
  ...
  await t.step(
    'GET /api/v1/news/:id - should handle missing news',
    async () => {
  ...
  await t.step(
    'GET /api/v1/news/:id - should increment read count',
    async () => {
      const newsId = 'news-1';
      const request = await superoak(app);
  ...
  await t.step(
    'GET /api/v1/news/trending - should return trending news',
    async () => {
      const request = await superoak(app);
      const response = await request
  ...
  await t.step(
    'GET /api/v1/news/trending - should respect limit parameter',
    async () => {
      const limit = 5;
      const request = await superoak(app);
      const response = await request
  ...
  await t.step(
    'GET /api/v1/news/categories - should return category statistics',
    async () => {
      const request = await superoak(app);
      const response = await request
      ...
      const body: ApiResponse<{ category: string; count: number }[]> =
        response.body;
...
Deno.test('News API Error Handling Tests', async (t) => {
  ...
  await t.step('should handle SQL injection attempts', async () => {
    ...
    const response = await request
      .get(
        `/api/v1/news?category=${encodeURIComponent(maliciousCategoryParam)}`,
      )
...
Deno.test('News API Performance Tests', async (t) => {
  ...
  await t.step('should respond within reasonable time', async () => {
    ...
    assertEquals(
      responseTime < 1000,
      true,
      `Response time ${responseTime}ms is too slow`,
    );
  });
});
